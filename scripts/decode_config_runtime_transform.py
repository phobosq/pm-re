#!/usr/bin/env python3
"""Decode config->runtime transform around 0x13C190 and callback 0x584A0.
Static only.
"""
...
TARGETS=[0x0013C190,0x000584A0,0x00058410,0x00145010]
INTEREST={0x2C0:'owner_config',0x90:'context',0x98:'mt',0xAC:'straps',0xB0:'vmr/rxboost',0xB8:'vmt2',0xBC:'vmt3'}
def main():
 ...
 lines=['# Config -> runtime transform decode','']
 for t in TARGETS:
  ...
  lines += ['```','']
 out=Path(a.out_dir);out.mkdir(parents=True,exist_ok=True);p=out/'config_runtime_transform.md';p.write_text('\n'.join(lines),encoding='utf-8');print(p)
